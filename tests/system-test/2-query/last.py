...
class TDTestCase:
    ...
    def last_check_scan_type(self, cacheModel):
        tdSql.execute("create database last_test_last_row_model cachemodel '%s';" % cacheModel)
        ...
        sql = f'select tbname, last(ts)  from test_t1;'
        tdSql.query(sql)
        tdSql.checkRows(1)
        ...
        sql = f'select last(ts), tbname from test_t1;'
        tdSql.query(sql)
        tdSql.checkRows(1)
        ...
        sql = f'select tbname, last(ts), tbname from test_t1;'
        tdSql.query(sql)
        tdSql.checkRows(1)
        tdSql.checkData(0, 0, "test_t1")     
        tdSql.checkData(0, 1, last_ts)  
