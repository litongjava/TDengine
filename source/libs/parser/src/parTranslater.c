...
static const SSysTableShowAdapter sysTableShowAdapter[] = {
  ...
  {
    .showType = QUERY_NODE_SHOW_ARBITRATORS_STMT,
    .pDbName = TSDB_INFORMATION_SCHEMA_DB,
    .pTableName = TSDB_INS_TABLE_ARBITRATORS,
    .numOfShowCols = 1,
    .pShowCols = {"*"}
  },
  {
...
static int32_t fillCmdSql(STranslateContext* pCxt, int16_t msgType, void* pReq) {
  ...
  switch (msgType) {
    ...
    case TDMT_MND_CREATE_ARBITRATOR: {
      FILL_CMD_SQL(sql, sqlLen, pCmdReq, SMCreateArbitratorReq, pReq);
      break;
    }
    case TDMT_MND_DROP_ARBITRATOR: {
      FILL_CMD_SQL(sql, sqlLen, pCmdReq, SMDropArbitratorReq, pReq);
      break;
    }
    default: {
      break;
...
static int16_t getCreateComponentNodeMsgType(ENodeType type) {
  switch (type) {
    ...
    case QUERY_NODE_CREATE_ARBITRATOR_STMT:
      return TDMT_MND_CREATE_ARBITRATOR;
    default:
      break;
  }
  return -1;
...
static int16_t getDropComponentNodeMsgType(ENodeType type) {
  switch (type) {
    ...
    case QUERY_NODE_DROP_ARBITRATOR_STMT:
      return TDMT_MND_DROP_ARBITRATOR;
    default:
      break;
  }
  return -1;
...
static int32_t rewriteQuery(STranslateContext* pCxt, SQuery* pQuery) {
  ...
  switch (nodeType(pQuery->pRoot)) {
    ...
    case QUERY_NODE_SHOW_ARBITRATORS_STMT:
      code = rewriteShow(pCxt, pQuery);
      break;
