...
static int32_t collectMetaKeyFromShowArbitrators(SCollectMetaKeyCxt* pCxt, SShowStmt* pStmt) {
  if (pCxt->pParseCxt->enableSysInfo) {
    return reserveTableMetaInCache(pCxt->pParseCxt->acctId, TSDB_INFORMATION_SCHEMA_DB, TSDB_INS_TABLE_ARBITRATORS,
                                   pCxt->pMetaCache);
  }
  return TSDB_CODE_SUCCESS;
}
...
static int32_t collectMetaKeyFromQuery(SCollectMetaKeyCxt* pCxt, SNode* pStmt) {
  ...
  switch (nodeType(pStmt)) {
    ...
    case QUERY_NODE_SHOW_ARBITRATORS_STMT:
      return collectMetaKeyFromShowArbitrators(pCxt, (SShowStmt*)pStmt);
