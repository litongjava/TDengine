...
typedef struct {
  int32_t  groupId;
  int8_t   replica;
  int32_t  dnodeIds[TSDB_MAX_REPLICA];
} SArbitratorGroupInfo;

typedef struct {
  int32_t      arbId;
  SArray*      groups;  // SArbitratorGroupInfo
} SArbitratorGroups, SArbRegisterGroupReq, SArbUnregisterGroupReq;

typedef struct {
  int32_t dnodeId;
  SArray* arbGroups; // SArbitratorGroups
} SMGetArbitratorsRsp;
...
typedef struct {
  int32_t groupId;
  int32_t seqNo;
  char    memberToken[TD_ARB_TOKEN_SIZE];
} SVArbHbMember;
...
typedef struct {
  ...
  char    arbToken[TD_ARB_TOKEN_SIZE];
  int32_t dnodeId;
  SArray* hbMembers; // SVArbHbMember
} SVArbHeartBeatRsp;
